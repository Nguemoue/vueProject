...
<div id="app">
    <v-app>
        ...
        <v-main app>
                ...
                <v-container>
                    <p><h2 class="text-center text-capitalize">Creation de Votre Compte</h2></p>
                    ...
                    <?php if($validator->hasError()): ?>
                        ...
                        <v-dialog max-width="400" :value="<?php echo "dialog"; ?>" permanent>
                            ...
                            <v-btn class="float-right" color="error" @click.stop=" dialog = false">
                                Fermer<v-icon right>mdi-delete</v-icon>
                            </v-btn>
                        </v-dialog>
                        <br>
                    <?php endif ?>
                    ...
                    <v-card>
                        <v-stepper :value="<?= $_SESSION['stepperStep'] ?>" >
                            <v-stepper-header>
                                <v-stepper-step step="1" <?php echo $validator->hasError()? ":rules='[()=>{return false}]'":":complete='true'";?>  >
                                    Creation
                                </v-stepper-step>
                                <v-divider></v-divider>
                            ...
                            <v-stepper-items>
                                <v-stepper-content step="1">
                                    <v-form dense v-model="validForm"  class="lighten-9  py-5" method="post"
                                        ...
                                        <v-text-field dense size="sm"  filled label="Nom" append-icon="mdi-account" required name="Username" :rules="rules.nom">
                                        </v-text-field>

                                        <v-text-field size="sm" dense filled label="Email" name="Email" append-icon="mdi-email" required type="email" :rules="rules.email"></v-text-field>

                                        <v-text-field :type="passwordVisible? 'text':'password'" dense name="Password"    filled label="Mot de Passe"  :append-icon=" passwordVisible?'mdi-eye':'mdi-eye-off' " @click:append=" passwordVisible = !passwordVisible" :rules="passwordRules" v-model="passwordModel">
                                        </v-text-field>
                                        {{ passwordModel==confirmPasswordModel }}
                                        <v-text-field :type="confirmPasswordVisible? 'text':'password'" dense
                                            name="ConfirmPassword"   filled label="Confirmer le Mot de Passe"   :append-icon=" confirmPasswordVisible ? 'mdi-eye':'mdi-eye-off'  " @click:append=" confirmPasswordVisible = !confirmPasswordVisible "  :rules="confirmPasswordRules"
                                        ...
                                        <v-select label="Genre" name="Gender" :items="['Homme','Femme']" :value="'Homme'"       filled  dense append-icon="mdi-human"></v-select>

                                        <v-autocomplete filled label="Ville" name="City"  dense v-model="ville"
                                            :items="villes"  append-icon="mdi-city"></v-autocomplete>

                                        <v-autocomplete label="Votre Pays" filled  :items="pays" dense v-model="pay"
                                            name="Country" ></v-autocomplete>

                                        <v-text-field name="Phone" filled type="number" :prefix="(codes[pay])?' +'+codes[pay]:'+ ...'" dense append-icon="mdi-phone" required></v-text-field>

                                        <v-file-input accept="image/*" chips counter show-size name="profil" ref="img"
                                            label="Votre Photo de Profile" prepend-icon=""  prepend-inner-icon="mdi-camera" dense filled truncate-length="50" :rules="rules.file" append-icon="mdi-account">
                                        </v-file-input>

                                        <!-- les boutons pour la soumissin du formulaire -->
                                        <v-btn type="reset" color="error" class="p-4 text-sm-subtitle-2"
                                            @click="$refs.form.reset();resetLoader=true;$refs.form.resetValidation()" :loading="resetLoading">
                                            Reinitialiser<v-icon right>mdi-delete</v-icon>
                                        </v-btn>
                                ...
                                <v-stepper-content step="2">
                                    ...
                                    <?php if($_SESSION['stepperStep'] == 2 and !isset($_POST['valider']) and !isset($_POST['coderesend']) ): ?>
                                        <v-alert type="success" prominent dismissible transition='scale-transition'>
                                    ...
                                    <?php if($_SESSION['stepperStep'] == 2 and (isset($_POST['coderesend']) )): ?>
                                        <v-alert type="success"   dismissible transition='scale-transition' class="text-subtitle-2">
                                    ...
                                    <?php if(isset($codeValid)): ?>
                                        <v-alert type="error" prominent dismissible transition='scale-transition'>
                                            Code Incorect
                                        </v-alert>
                                    <?php endif ?>
                                    <v-form method="post" action="<?=$_SERVER['PHP_SELF']?>">
                                        ...
                                        <v-row>
                                            ...
                                            renvoyer le code <v-icon>mdi-share</v-icon>
                                        </v-btn>
                                            </v-col cols="12">
                                            <v-col>
                                        <v-btn class="float-right" type="submit" color="success" name="valider">
                                            Valider <v-icon right>mdi-send</v-icon>
                                        </v-btn>
